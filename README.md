...
# Chrono 编程语言 (Chrono Lang)

**"The TypeScript for C++"**

## 1\. 项目概述

**Chrono** 是一个现代化的编程语言项目，旨在解决 C++ 开发中的痛点，同时保留其零开销的性能优势。它不是一个运行在虚拟机上的解释型语言，而是一个 **智能转译器 (Smart Transpiler)**。

Chrono 源代码 (`.ch`) 会被 100% 翻译为现代标准 C++17 代码 (`.cpp` / `.h`)，生成的代码可以直接集成到任何现有的 C++ 项目中，并使用 MSVC、GCC 或 Clang 进行编译。

### 核心特性

  * **声明与实现分离**：通过 `class` 定义接口，通过 `implement` 块编写逻辑，彻底告别手动维护头文件的繁琐。
  * **双重内存模型**：
      * **MRC (手动引用计数)**：通过 `ChronoObject` 提供类似 Objective-C 的引用计数管理，适合高性能场景。
  ...
  * **无缝 C++ 互操作**：
      * **零开销调用**：Chrono 类型直接映射为 C++ 原生类型（如 `i32` -\> `int32_t`）。
...
│   ├── ChronoVisitor.py    # [核心] AST 遍历与代码生成逻辑
│   ├── CompileContext.py   # 编译上下文管理
│   └── parser/             # ANTLR4 生成的 Lexer 和 Parser
├── runtime/                # Chrono 运行时库 (C++ 头文件)
│   ├── ChronoObject.h      # MRC 基类
│   ├── Chrono.h            # 工具函数与日志
│   └── ...
...
`build.py` 读取 `chrono.json` 配置，递归扫描源码目录并批量转译。

1.  确保根目录存在 `chrono.json`：
    ```json
    {
        "src_dir": "src",
        "out_dir": "build/src",
...
// 类型推导 (auto)
var name = "Chrono"; // 推导为 const char*
const MAX_COUNT = 100;
...
| Chrono | C++ (stdint) | Chrono | C++ (Native) |
| :--- | :--- | :--- | :--- |
...
Chrono 强制将 **声明 (Declaration)** 与 **实现 (Implementation)** 分离。
...
通常继承自 `ChronoObject`，支持继承和虚函数，默认 `private`。

```chrono
class Shape : ChronoObject {
    var area: f32;
...
// 使用 'with' 关键字实现接口
class Button : ChronoObject with IDrawable {
...
Chrono 将 C++ 智能指针提升为语言级的一等公民。

#### 语法糖

| 概念 | Chrono 写法 | 生成 C++ 代码 |
| :--- | :--- | :--- |
...
import "runtime/ChronoObject.h"   // 用户头文件
...
1.  **修改语法**：编辑 `src/parser/ChronoLexer.g4` 或 `ChronoParser.g4`。
2.  **重新生成 Parser**：运行 `gen.bat` (需要 Java 环境)。
3.  **修改逻辑**：编辑 `src/ChronoVisitor.py`。
