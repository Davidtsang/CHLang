    ...
    virtual AnyVar msgSend(SelectorID sel, ArgsList args) {
        ...
        if (method) {
            try {
                ...
                std::cerr << "[CHRuntime] Type mismatch: " << e.what() << std::endl;
            }
        } else {
            // 没找到
            // 这里可以打印出 sel 的整数 ID，虽然不可读，但在调试器里能看
            std::cerr << "[CHRuntime] Selector ID " << sel << " not recognized." << std::endl;
        }
        return {};
    }
